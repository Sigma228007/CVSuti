...
function PageInner() {
  ...
  // грузим баланс только в Telegram
  useEffect(() => {
    let stop = false;
    async function load() {
      if (!insideTg || !initData) { setBalance(null); return; }
  ...
  async function handlePay() {
    ...
    try {
      ...
      if (!r.ok || !data?.ok || !data?.url || !data?.id) throw new Error(data?.error || `Server error (${r.status})`);

      // Открываем кассу СЕЙЧАС (в клике)
      ...
      <div style={{ marginTop: 8, background: '#0f1720', padding: 20, borderRadius: 12, maxWidth: 900, boxShadow: '0 6px 24px rgba(0,0,0,0.25)' }}>
        <div style={{ marginBottom: 12 }}>
          <span style={{ fontSize: 14, background: '#111827', padding: '6px 10px', borderRadius: 8, color: '#93c5fd' }}>
            {insideTg ? (balance === null ? 'Баланс: —' : `Баланс: ${balance} ₽`) : 'Баланс: — (откройте через Telegram)'}
          </span>
        </div>
        ...
        <p style={{ marginTop: 4, marginBottom: 14, color: '#9aa9bd', lineHeight: 1.45 }}>
          Оплата откроется во внешнем браузере. После успешной оплаты вернитесь в Telegram и перезапустите мини-приложение — баланс обновится.
        </p>
